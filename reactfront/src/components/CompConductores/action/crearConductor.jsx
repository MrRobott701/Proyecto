...
const CompCreateConductores = ({ onClose, getConductores }) => {
  ...
  const handleIneSelected = (file) => {
    const renamedFile = new File([file], formatFileName(file.name, '_INE'), { type: file.type });
    setSelectedIneFile(renamedFile);
  };

  const handleLicSelected = (file) => {
    const renamedFile = new File([file], formatFileName(file.name, '_LIC'), { type: file.type });
    setSelectedLicFile(renamedFile);
    };

    const handleReciboLuzSelected = (file) => {
      const renamedFile = new File([file], formatFileName(file.name, '_RECIBO_LUZ'), { type: file.type });
      setSelectedReciboLuzFile(renamedFile);
    };

    const handleReciboAguaSelected = (file) => {
      const renamedFile = new File([file], formatFileName(file.name, '_RECIBO_AGUA'), { type: file.type });
      setSelectedReciboAguaFile(renamedFile);
    };

    const handleAvalDocSelected = (file) => {
      const renamedFile = new File([file], formatFileName(file.name, '_AVAL_DOC'), { type: file.type });
      setSelectedAvalDocFile(renamedFile);
    };

    const handleAvalLuzSelected = (file) => {
      const renamedFile = new File([file], formatFileName(file.name, '_AVAL_LUZ'), { type: file.type });
      setSelectedAvalLuzFile(renamedFile);
    };

    const handleAvalAguaSelected = (file) => {
      const renamedFile = new File([file], formatFileName(file.name, '_AVAL_AGUA'), { type: file.type });
